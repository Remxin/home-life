...
class GrpcGatewayClient {
  ...
  private static async makeRequest<T, R>(path: string, include_headers: AsyncStorageKeys[], method: APIMethods, body: T): Promise<[HomeLifeRpcError | null, R | null]> {
    return new Promise(async (resolve) => {
      ...
      for (let newHeader of include_headers) {
        ...
        if (newHeader === "access_token") headers["authorization"] = `bearer ${headerValue}`
  ...
  static async loginUser(
    ...
    return this.makeRequest<{ email: string, password: string}, LoginUserResponse>("/v1/login_user", [], "POST", {
      email, password
    })
  }

  static async getFamily(): Promise<[HomeLifeRpcError | null, GetFamilyResponse | null]> {
    return this.makeRequest<{}, GetFamilyResponse>("/v1/family", ["access_token", "permission_token"], "GET", {})
  }

  static async createFamily(family_name: string): Promise<[HomeLifeRpcError | null, CreateFamilyResponse | null]> {
    return this.makeRequest<{}, CreateFamilyResponse>("/v1/family", ["access_token"], "POST", {
      family_name
    })
  }
}
